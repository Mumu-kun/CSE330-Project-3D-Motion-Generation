...
class IgniteMotionTrainer:
    ...
    def _compute_loss(
        ...
        *,
        ...
        target_flow = subset_271d_to_68d(
            target_frame,
    ...
    def _compute_jepa_loss(
        ...
        *,
        ...
        context_all = encoder(motion, text_embedding, return_all=True)

        batch_size, seq_len, hidden_size = context_all.shape

        num_masked = max(1, int(seq_len * mask_ratio))
        perm = torch.randperm(seq_len, device=self.device)
        mask_indices = perm[:num_masked]

        motion_masked = motion.clone()
        motion_masked[:, mask_indices] = 0.0

        masked_context_all = encoder(
            motion_masked, text_embedding, return_all=True
        )

        masked_context = masked_context_all[:, mask_indices, :]
        target_context = context_all[:, mask_indices, :].detach()

        predicted = jepa_predictor(
            masked_context.reshape(batch_size * num_masked, hidden_size)
        )

        return F.mse_loss(predicted, target_context.reshape(batch_size * num_masked, hidden_size))
    ...
    def run(
        ...
        trainer.run(train_loader, max_epochs=max_epochs or int(self.config.num_epochs))
        return trainer
