...
class Trainer:
    ...
    def __init__(
        ...
        self.total_epochs = int(config.get_num_epochs)
        self.checkpoint_id = (
    ...
    def _resolve_total_epochs_from_checkpoint(self, checkpoint: Dict[str, Any]) -> int:
        ...
        if total_epochs is None:
            ...
            if isinstance(checkpoint_config, Config):
                total_epochs = checkpoint_config.get_num_epochs
            else:
                total_epochs = self.config.get_num_epochs
    ...
    def _compute_post_warmup_progress(self, epoch: int) -> float | None:
        ...
        if self.config.get_num_epochs <= 1:
            return 1.0

        progress = float(epoch) / float(self.config.get_num_epochs - 1)
    ...
    def _compute_t_sampling_power(self, epoch: int) -> float:
        ...
        if target_power == 0.0 or self.config.get_num_epochs <= 1:
            return target_power
        ...
        if self.config.use_consistency_loss:
            ...
            if t_thresh_mask.any():
                with timer(self.timing_stats, "forward/consistency_loss"):
                    ...
                    flow_output_raw = (
                        self.normalizer.denormalize_x68(pred_x1)
                        if self.normalizer is not None
                        else pred_x1
                    )
                    x1_raw = (
                        self.normalizer.denormalize_x68(x1[t_thresh_mask])
                        if self.normalizer is not None
        ...
        def generate_rollout_frame(
            *,
            ...
            with torch.no_grad():
                ...
                pred_flow_raw = (
                    self.normalizer.denormalize_x68(x_t)
                    if self.normalizer is not None
                    else x_t
                )
                current_frame_raw = (
                ...
                next_positions_local = x68_to_positions(
                    pred_flow_raw,
                    prev_root_pos=current_positions_local[:, 0],
                    prev_root_rot_6d=current_frame_raw[:, 69:75],
                )
                next_frame_local, _, fk_positions = generated_positions_to_x271(
    ...
    def setup_training_environment(self) -> None:
        ...
        num_epochs = int(self.config.get_num_epochs)
        ...
        if self.resume_from is not None and os.path.exists(self.resume_from):
            ...
            self.config.get_num_epochs = self.total_epochs
